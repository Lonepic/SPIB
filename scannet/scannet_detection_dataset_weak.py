...
class ScannetLabedledTwoStreamDataset(Dataset):
    ...
    def __init__(self, split_set='train',labeled_sample_list=None, num_points=20000, use_color=False, use_normal=False, use_height=False, augment=False):
    ...
    def __getitem__(self, idx):
        ...
        #    
        #if self.use_normal:
        #    n = pcu.estimate_point_cloud_normals(raw_point_cloud[:,0:3], k=16)
        #    raw_point_cloud = np.c_[raw_point_cloud,n]
#
        #if self.use_height:
        ...
        raw_point_cloud = mesh_vertices[:, 0:3]
        
        floor_height = np.percentile(raw_point_cloud[:, 2], 0.99)
        height = raw_point_cloud[:, 2] - floor_height
        raw_point_cloud = np.concatenate([raw_point_cloud, np.expand_dims(height, 1)], 1)
        
        raw_point_cloud = np.c_[raw_point_cloud,(mesh_vertices[:, 3:6] - MEAN_COLOR_RGB) / 256.0]
        ...
        semantic_label = np.zeros(len(semantic_labels))
        for i in range(0, len(semantic_labels)):
            if semantic_labels[i] in DC.nyu40ids:
                semantic_label[i]= DC.nyu40id2class[semantic_labels[i]]
            else:
                semantic_label[i]= -1  
        ret_dict['semantic_labels'] = semantic_label.astype(np.int64) 
        # *********************************

        return ret_dict

class ScannetUnlabedledTwoStreamDataset(Dataset):
    def __init__(self, labeled_sample_list=None, num_points=20000, use_color=False, use_normal=False, use_height=False, augment=False):
